...
def img_writer(raw_masks):
    ...
    if rgb:
        ...
        out = os.path.join(output_path, "pre_p", f"{mask_name}.png")
        pal_img = Image.fromarray(mask.astype("uint8"), mode="P")
        pal_img.putpalette(bytearray([c for rgb in PALETTE for c in rgb]))  # flatten
        pal_img.save(out)